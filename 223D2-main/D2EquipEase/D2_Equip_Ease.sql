...
--insert statments for 4.1
--INSERT INTO Users VALUES (\'" + username + "\', \'" + firstname + "\', \'" + middlename + "\', \'" + lastname + "\', \'" + password + "\')");
...
IF NOT EXISTS (SELECT 1 FROM Customer WHERE Email = 'Bob@example.com')
BEGIN
    INSERT INTO Customer (Email, fname, sname, phone)
    VALUES ('Bob@example.com', 'Bob', 'Smith', '555-555-5555'); -- Replace with actual details if available
END


--Declear variables
DECLARE @EquipmentId INT;
DECLARE @TypeName VARCHAR(225) = 'Jigsaw';
DECLARE @BranchName VARCHAR(30) = 'Hamilton East Branch'


--check if equipment available 
SELECT TOP 1 @EquipmentId = EquipmentID
FROM Equipment
